...
const generateFileDocument = () => {
    ...
    return Yup.array()
        ...
        .test("fileFormat", "Hanya file JPG, JPEG, PNG, SVG dan PDF yang diizinkan", (files) => {
            ...
            return !invalidFiles;
        }).test("fileSize", `Ukuran file / dokumen tidak boleh lebih dari ${maxSizeMb} MB`, (files) => {
            if (!files || files.length === 0) {
                return true
            }
...
const generateFileDocumentRequired = () => {
    const allowedFileFormats = ["jpg", "jpeg", "png", "svg", "pdf"];
    const maxSizeMb = 3
    const maxSizeBytes = maxSizeMb * 1024 * 1024

    return Yup.array()
        .min(1, "File/Image wajib diisi") // Validasi minimal 1 file (required)
        .test(
            "fileFormat",
            "Hanya file JPG, JPEG, PNG, SVG dan PDF yang diizinkan",
            (files) => {
                if (!files || files.length === 0) {
                    return false; // Gagal jika tidak ada file
                }
                const invalidFiles = files.some((file) => {
                    const fileName = file.name.toLowerCase();
                    const fileExtension = fileName.split(".").pop();
                ...
                return !invalidFiles; // Validasi berhasil jika tidak ada file tidak valid
            }
        ).test("fileSize", `Ukuran file / dokumen tidak boleh lebih dari ${maxSizeMb} MB`, (files) => {
            if (!files || files.length === 0) {
                return false
...
const formPengurusanSchema = Yup.object({
    nama_pic: Yup.string().required("Nama Lengkap wajib di isi"),
    nomor_pic: Yup.string()
        .matches(phoneRegExp, "No. Handphone / WhatsApp tidak valid")
        .required("No. Handphone / WhatsApp wajib di isi"),
    upload_ktp: generateFileDocumentRequired(),
    upload_pas_foto: generateFileDocumentRequired(),
    upload_skck: Yup.mixed().when("currentPath", {
        is: (path) => path.endsWith("perpanjangan"), // Jika currentPath berakhiran "perpanjangan"
        then: generateFileDocumentRequired(), // Wajib diisi
        otherwise: generateFileDocument(), // Tidak wajib
    }),
    ketentuan_cek: Yup.boolean().oneOf([true], "Ketentuan wajib di isi"),
});
