...
function ContentDetailCheckoutP1({ layanan, data }) {
  ...
  useEffect(() => {
    if (layanan === "PWAN") {
      const fetchPWAN = () => {
        ...
        const convertJsonComponent = JSON.parse(data?.history?.json_others_component)
        setJsonData(convert)
        setJsonComponent(convertJsonComponent)
      }
      fetchPWAN()
    }
  }, [layanan, data]);
  ...
  let technical_meeting = 0;

  if (data?.history?.technical_meeting === "true") {
    if (totalPayment >= 1000000 && totalPayment <= 4000000) {
      technical_meeting = 200000
    } else if (totalPayment >= 5000000 && totalPayment <= 10000000) {
      technical_meeting = 300000
    } else if (totalPayment >= 10000000) {
      technical_meeting = 500000
    }
  } else {
    technical_meeting = 0;
  }
  ...
  totalAkhir = totalPayment + technical_meeting
          ...
          <>
            ...
            <div>
                    ...
                    <div key={komponenIndex} className="row">
                      ...
                      <div className="col-md-4">
                        <div style={{ ...stylesValue.titleValue, fontWeight: 'bold', textAlign: 'right' }}>
                          ...
                          {`Rp. ${(komponen.harga).toLocaleString('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} `} / ({komponen.unit})
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="row">
                ...
                <div className="col-md-4">
                  <div style={{ ...stylesValue.titleValue, fontWeight: 'bold', textAlign: 'right' }}>
                    {`Rp. ${(totalPayment).toLocaleString('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} `}
                  </div>
                </div>
              </div>
              ...
              <div className="row">
                ...
                <div className="col-md-4">
                  <div style={{ ...stylesValue.titleValue, fontWeight: 'bold', textAlign: 'right' }}>
                    {`Rp. ${(totalAkhir).toLocaleString('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} `}
                  </div>
                </div>
              </div>
